import string
import operator
import sys

def MostTweeted():
    INPUTFILE = input("Enter the path of the input file located: ")
    INPUT_FILE_PATH = INPUTFILE+ '.txt'
    A=input("Number of record: ")
    N=int(A)
    with open (INPUT_FILE_PATH, encoding = "latin-1") as myFile:
        twitter=myFile.readlines()
    l = {}
    for dat in twitter:
       
        fileTemp = dat.split()
        if fileTemp[0] in l:
            l[fileTemp[0]] +=1
        else:
            l[fileTemp[0]] = 1
    l = sorted(l.items(), key = operator.itemgetter(1), reverse = True)
 
    outputFile = open('J:/SSDI/MostTweeted.txt', 'w', encoding = "utf-8")
    outputFile.write("The top " + A + " users who have tweeted the most for the entire timeline: \n")
    for i in range (0,N):
        outputFile.write("The user " + l[i][0] + " tweeted " + str(l[i][1]) + " times" + "\n")
        
    outputFile.close
MostTweeted()

def Tweetedperhour():
    INPUTFILE = input("Enter the path of the input file located: ")
    INPUT_FILE_PATH = INPUTFILE+ '.txt'
    A=input("Number of record: ")
    N=int(A)
    
    with open (INPUT_FILE_PATH, encoding = "latin-1") as myFile:
        data=myFile.readlines()

    g = {}
    for dat in data:
        fileTemp = dat.split()
        fileTemp2 = fileTemp[1].split(":")
        twitTemp = fileTemp[0] + " " + fileTemp2[1]
        if twitTemp in g:
            g[twitTemp]+=1
        else:
            g[twitTemp]=1
    g = sorted(g.items(), key = operator.itemgetter(1), reverse = True)

    g2={}
    totalUsersIn = 0
    for dat in g:
        totalUsersIn+=1
        if(dat[0].split()[1]) in g2:
            g2[dat[0].split()[1]]+=1
        else:
            g2[dat[0].split()[1]]=1
    g2 = sorted(g2.items(), key = operator.itemgetter(1))

    totalEntriesToPrint = N*len(g2)
    outputFile = open('J:/SSDI/Tweetedperhour.txt', 'w', encoding = "utf-8")
   
    outputFile.write("The top " + A + " users who have tweeted the most for every hour: " + "\n\n")
    for x in range (0,len(g2)):
   
        mSearch = N
        
       
        for dat in g:
            if mSearch == 0:
                break
            if dat[0].split()[1] == g2[x][0]:
                outputFile.write("Username: " + dat[0].split()[0] + "\n\n")
                
                mSearch-=1
    outputFile.close
Tweetedperhour()



def Followerstweeted():
    INPUTFILE = input("Enter the file path: ")
    INPUT_FILE_PATH = INPUTFILE+ '.txt'
    A=input("Number of record: ")
    N= int(A)
    with open (INPUT_FILE_PATH, encoding = "latin-1") as myFile:
        twitter=myFile.readlines()

    l = {}
    for dat in twitter:
        fileTemp = dat.split()
        if fileTemp[0] not in l:
            l[fileTemp[0]] = int(fileTemp[-2])

    l = sorted(l.items(), key = operator.itemgetter(1), reverse = True)
    outputFile = open('J:/SSDI/Followerstweeted.txt', 'w', encoding = "utf-8")
    outputFile.write("The top " + A + "users who has the maximum followers: " + "\n\n")

    for i in range (0, N):
        outputFile.write(str(i+1) + ". Username: " + l[i][0] + " -> Number of Followers: " + str(l[i][1]) + "\n\n")
    outputFile.close
Followerstweeted()

def countofretweet():
    INPUTFILE = input("Enter the path of the input file located: ")
    INPUT_FILE_PATH = INPUTFILE+ '.txt'
    A=input("Number of record: ")
    N=int(A)
    with open (INPUT_FILE_PATH, encoding = "latin-1") as myFile:
        twit=myFile.readlines()

    g = {}
    for dat in twit:
  
        fileTemp = dat.split()
        y = len(fileTemp)-2
        tweet = "\""
        for x in range (4, y):
            tweet += fileTemp[x] + " "
        tweet += " ::::;:::: " + fileTemp[0]
  
        if tweet not in g:
            g[tweet] = int(fileTemp[-1])

    outputFile = open('J:/SSDI/countofretweet.txt', 'w', encoding = "utf-8")
	   
    g = sorted(g.items(), key = operator.itemgetter(1), reverse = True)
    outputFile.write("The top "+ A + "tweets that have the max retweet count: " + "\n\n")

    for x in range (0, N):
        outputFile.write("\n Tweet: " +
                      g[x][0].split("::::;::::")[0]  + "\n\n")
    outputFile.close
countofretweet()

